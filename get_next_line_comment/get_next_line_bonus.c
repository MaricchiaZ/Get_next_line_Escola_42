...
/*   Updated: 2022/10/07 20:12:29 by maclara-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd) // a função de leitura recebe o fd passado por parâmetro
{
	...
	static char	*store_rest[FD_MAX] // array de arrays, assim cada fd terá seu ponteiro, permitindo que a função
	// seja utilizada para ler vários arquivos ao mesmo tempo, os ponteiros para a string que armazenarão o resto da linha, 
	...
	store_rest[fd] = separete_line_rest(line); //essa função midifica a linha, pois apara a linha no primeiro "\n" encontrado, e o restante, armazena
	// no store_rest; Cada fd estará em uma posisão do array de arrays, fazendo com que não se confundam
